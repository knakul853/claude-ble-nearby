...
connectionManager.on('disconnected', (id: string) =>
  peerManager.onDisconnected(id),
);
...
server.tool('ble_scan_all', 'List ALL nearby BLE devices (for debugging). Shows both Claude Code peers and other BLE devices.', {}, async () => {
  ...
  return {
    content: [{ type: 'text' as const, text: JSON.stringify({ devices, total: devices.length }) }],
  };
});
...
server.tool(
  ...
  async ({ peerId }) => {
    ...
    if (!peer) {
      return {
        content: [
          { type: 'text' as const, text: JSON.stringify({ error: 'Peer not found' }) },
        ],
      };
    }
    ...
    return {
      content: [
        { type: 'text' as const, text: JSON.stringify({ sent: true, to: peer.name }) },
      ],
    };
  },
);
...
server.tool(
  ...
  async ({ peerId, accept }) => {
    ...
    return {
      content: [{ type: 'text' as const, text: JSON.stringify({ paired: accept }) }],
    };
  },
);
...
server.tool(
  ...
  async ({ peerId, message }) => {
    if (!store.isPaired(peerId)) {
      return {
        content: [
          { type: 'text' as const, text: JSON.stringify({ error: 'Peer not paired' }) },
        ],
      };
    }
    ...
    if (connectionManager.isConnected(peerId)) {
      ...
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ delivered: true }) }],
      };
    }
